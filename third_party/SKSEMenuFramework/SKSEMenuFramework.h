...
inline HMODULE SKSEMenuFrameworkModule() {
    static HMODULE module = nullptr;
    if (!module) {
        module = GetModuleHandle(L"SKSEMenuFramework");
    }
    return module;
}
#define menuFramework SKSEMenuFrameworkModule()
...
namespace SKSEMenuFramework {
    inline ImTextureID LoadTexture(std::string texturePath, ImVec2 size = ImVec2(0.0f, 0.0f)) {
        using func_t = ImTextureID (*)(const char*, ImVec2);
        static auto func = Internal::GetFunction<func_t>("LoadTexture");
        return func ? func(texturePath.c_str(), size) : nullptr;
    }
}
